...
public class Robot {
	...
	/**
	 * une liste permettant d'enregistrer les depacemment du robot
	 */
	public ArrayList<String> maListe ;
	...
	public boolean verification(double maBatterie) {
		if(maBatterie >= 30 && maBatterie <= 36) {
			return true ;
		}
		else {
			return false ;
	...
	public void retourCasedepart(int ligne , int colonne , String direction , ArrayList<String> maListe)
	{
		if("sud".equals(direction))
		{
			tournerDroite();maListe.add("D");
			tournerDroite();maListe.add("D");
		}
			...
			tournerGauche();maListe.add("G");
		}
		
		for(int i = 0 ; i < ligne ; i++) {
			if(isObstacleDevant()) {
				tournerDroite();maListe.add("D");
                if (!isObstacleDevant()) avancer();maListe.add("A");
                i++;
                tournerGauche();maListe.add("G");
                if (!isObstacleDevant()) avancer();maListe.add("A");
                if (!isObstacleDevant()) avancer();maListe.add("A");
	...
	public void retourPosition(ArrayList<String> maListe) {
		for(int i = maListe.size()-1 ; i >= 0 ; i--)
		{
			if("A".equals(maListe.get(i)))
			{
				reculer();
			}
			else if("D".equals(maListe.get(i)))
			{
				tournerGauche();
			}
			else if ("G".equals(maListe.get(i)))
			{
				tournerDroite();
			}
